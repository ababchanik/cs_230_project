...
def one_step_strain_random_walk(rng,
                                inc_min: float = 1e-6, 
                                inc_max: float = 1e-4):
    """
    ...
    mags  = rng.uniform(inc_min, inc_max, size=(6,))
    deps  = signs * mags
...
def build_dataset(m: int,
        ...
        for t in range(T):
            ...
            if plastic_reached:
                ...
                while not incr_found and tries < max_tries:
                    ...
                    deps_rw_cur = one_step_strain_random_walk(rng=rng, inc_min=inc_min, inc_max=inc_max)
    ...
    return eps, deps, epse, sig, phase, sig_star
...
def parse_args():
    ...
    ap.add_argument("--out", type=str, default="data_stress_strain_random_then_fixed.npz")
    return ap.parse_args()

def main():
    args = parse_args()
    eps, deps, epse, sig, phase, sig_star = build_dataset(args.m, args.T, args.inc_min, args.inc_max, args.seed, args.max_tries)
    np.savez(args.out, eps=eps, deps=deps, epse=epse, sig=sig, phase=phase, sig_star=sig_star, meta=dict(m=args.m, T=args.T, seed=args.seed))
