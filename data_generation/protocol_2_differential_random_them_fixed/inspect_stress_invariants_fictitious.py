...
def main():
    ...
    if "sig_star" not in d.files: 
        raise KeyError(f"'sig_star' missing; keys: {d.files}")
    sig_star = d["sig_star"]                    # (m, T, 6) make mistakes. Check important info.
    if sig_star.ndim!=3 or sig_star.shape[-1]!=6: 
        raise ValueError(f"sig_star shape {sig_star.shape} not (m, T, 6)")
    m, T, _ = sig_star.shape
    p_star, q_star = p_q_from_voigt(sig_star)        # (m, T)
    keep = np.arange(m)
    ...
    # q–p paths
    for i in idxs:
        pi, qi = p_star[i, :], q_star[i, :]
        ax.plot(pi, qi, marker="x", ms=3, lw=args.lw, alpha=args.alpha, label="fictitious")
        ax.scatter(pi[0],  qi[0],  s=20, marker="o", color="blue")
        ax.scatter(pi[-1], qi[-1], s=20, marker="s", color="red")
        '''
        pi, qi = p[i, :], q[i, :]
        ax.plot(pi, qi, marker="x", ms=3, lw=args.lw, alpha=args.alpha, label="real")
        ax.scatter(pi[0],  qi[0],  s=20, marker="o", color="blue")
        ax.scatter(pi[-1], qi[-1], s=20, marker="s", color="red")
        '''
