...
This script:
  ...
  Figure 2 (elastic strain–strain, 2×3 panels):
...
def main(args):
    ...
    required_pred = ["sig_pred", "epse_pred", "L", "H_model", "H_test", "t0"]
    ...
    epse_pred_np = roll["epse_pred"]   # (H_test, 6)
    ...
    print(f"       epse_pred shape = {epse_pred_np.shape}")

    # ------------------------------------------------------------
    # Load full ground-truth trajectory
    # ------------------------------------------------------------
    truth = np.load(args.data_truth)
    required_truth = ["eps", "epse", "deps", "sig"]
    ...
    epse_all = truth["epse"]   # (T,6) or (1,T,6)
    ...
    # We still use sig_all/epse_all over [0,T) for "entire horizon" truth curves.
    ...
    # For Figure 2 (epse), define nicer labels
    epse_tex = [r'\varepsilon_{e,11}', r'\varepsilon_{e,22}', r'\varepsilon_{e,33}',
                r'\varepsilon_{e,23}', r'\varepsilon_{e,13}', r'\varepsilon_{e,12}']
    ...
    # ------------------------------------------------------------
    # Figure 2: Elastic strain–time comparison
    #   - epse truth over [0, T)
    #   - epse prediction over [t0, t0+H_test)
    #   x-axis = time step index
    # ------------------------------------------------------------
    fig2, axes2 = plt.subplots(2, 3, figsize=(10, 6), constrained_layout=True)
    fig2.suptitle(
        "Figure 2: Elastic strain vs time step (Voigt components)\n"
        f"truth: [0, T={T}), prediction: [t0={t0}, t0+H_test={t0+H_test}) "
        f"(L={L}, H_model={H_model}, H_test={H_test})",
        fontsize=12
    )
    ...
    for idx, ax in enumerate(axes2.ravel()):
        ...
        # Truth over entire horizon: epse vs time step
        y_full_epse = epse_all[:, I]           # ε_{e,I}^truth over [0,T)

        # Prediction over rollout horizon: epse vs time step
        y_pred_epse = epse_pred_np[:, I]       # ε_{e,I}^pred over [t0,t0+H_test)

        ax.plot(t_full, y_full_epse, '-o', lw=1.0, ms=3,
                label="ground truth (epse, full)")
        ax.plot(t_roll, y_pred_epse,  '-s', lw=1.0, ms=3,
                label="prediction (epse, rollout)")

        ax.set_xlabel(r'$t$ (time step)')
        ax.set_ylabel(rf'${epse_tex[idx]}$')
        ax.set_title(rf'${epse_tex[idx]}$ vs time step', fontsize=10)
        ax.tick_params(labelsize=8)
    ...
    out_path2 = os.path.join(args.out_dir, f"epse_time_rollout_voigt_t0_{t0:04d}.png")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--data_truth", type=str, required=True, help="Path to ground-truth NPZ with eps, epse, deps, sig.")
    ap.add_argument("--data_pred", type=str, required=True, help="Path to rollout NPZ with sig_pred, epse_pred, L, H_model, H_test, t0.")
