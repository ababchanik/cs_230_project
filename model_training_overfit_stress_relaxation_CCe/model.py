...
class SeqModel(nn.Module):
    """
    Per-step inputs (features): total strain eps(6) + elastic strain epse(6) + increment deps(6) + trial stress(6) = 24
    ...
    def _step_features(self, eps, epse, deps, sig_trial):
        # eps, epse, deps: (B, L, 6)
        return torch.cat([eps, epse, deps, sig_trial], dim=-1)  # (B, L, 24)

    def forward(self, eps_seq, epse_seq, deps_seq, sig_trial_seq):
        # eps_seq, epse_seq, deps_seq: (B, L, 6)
        # X from (t - L + 1) to (t)
        X = self._step_features(eps_seq, epse_seq, deps_seq, sig_trial_seq)  # (B, L, 24)
        # h at (t)
        h = self.enc(X)                                                      # (B, hid)
        # Y from (t + 1) to (t + H)
        Y = self.dec(h)                                                      # (B, H, 12)
