...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for eps, epse, deps, sig_y, epse_y, t0 in tr:
            ...
            # eps: (B,L,6), CCe_voigt: (6,6) → sig_trial_in: (B,L,6)
            sig_trial = elastic_update(eps, CCe_voigt)
            # Forward
            sig_pred, epse_pred = model(eps, epse, deps, sig_trial)          # (B,H,6) each
    ...
    with torch.no_grad():
        ...
        # Rebuild σ_trial for this batch
        sig_trial = elastic_update(eps, CCe_voigt)                 # (B,L,6)

        sig_pred, epse_pred = model(eps, epse, deps, sig_trial)
