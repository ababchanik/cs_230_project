...
def main(args):
    ...
    for k in range(n_blocks):
        ...
        eps_hist  = torch.from_numpy(eps_all[in_start:in_end]).float().unsqueeze(0).to(device)
        ...
        deps_future = torch.from_numpy(deps_all[f_start:f_end]).float().unsqueeze(0).to(device)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--P", type=int, default=None, help="Stride in rollout horizon between model calls (1 ≤ P ≤ H)." \
