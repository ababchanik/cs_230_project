...
Multi-trajectory capable version:

  • Ground-truth NPZ (data_truth) may contain:
        sig : (T, 6)          single trajectory
        or sig : (m, T, 6)    multiple trajectories

  • Rollout NPZ (data_pred) contains:
        sig_pred : (H_test, 6)
        L, H_model, H_test, t0
        optionally i_spec : int (index of specimen used for rollout)

  • If data_pred has "i_spec", we use that specimen from data_truth.
    Otherwise, we use CLI arg --i_spec (default 0).
...
# ============================================================
#   Main: plot-only p–q for a chosen trajectory
# ============================================================
def main(args):
    # ------------------------------------------------------------
    ...
    # Determine which specimen index to use
    if "i_spec" in roll.files:
        i_spec_roll = int(roll["i_spec"])
        print(f"[INFO] Using i_spec from rollout file: i_spec={i_spec_roll}")
        i_spec = i_spec_roll
    else:
        i_spec = args.i_spec
    ...
    sig_all = data["sig"]   # (T,6) or (m,T,6)

    # Allow both (T,6) and (m,T,6)
    if sig_all.ndim == 2:
        # Treat single-trajectory as m=1
        sig_all = sig_all[None, ...]

    m, T, D = sig_all.shape
    if D != 6:
        raise RuntimeError(f"Expected Voigt-6 last dimension (6), got D={D}")

    if not (0 <= i_spec < m):
        raise RuntimeError(f"Requested i_spec={i_spec} out of range for m={m}")

    sig_all_spec = sig_all[i_spec]   # (T,6) for chosen specimen
    print(f"[INFO] Ground-truth: m={m}, T={T}, using specimen i_spec={i_spec}")
    print(f"[INFO] Ground-truth trajectory length T={T}")

    # sanity-check that rollout horizon fits in T
    if t0 + H_test > T:
        raise RuntimeError(f"Rollout horizon t0+H_test = {t0 + H_test} exceeds T={T}.")

    # Full ground truth 0..T-1 for chosen specimen
    sig_true_full = sig_all_spec.copy()          # (T,6)
    ...
    sig_true_win = sig_all_spec[sl_out].copy()   # (H_test, 6)
    _ = sig_true_win  # silence unused var warning
    ...
    ax.set_title(
        f"q–p path (relaxation, i_spec={i_spec}, t0={t0}, "
    ...
    fname = os.path.join(args.out_dir, f"pq_rollout_i{i_spec}_t0_{t0:04d}.png")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        "Plot invariant p–q trajectories for a relaxation path (rollout vs truth), "
        "multi-trajectory capable."
    )
    ap.add_argument(
        "--data_truth", type=str, required=True,
        help="Ground-truth NPZ with eps, epsp, deps, sig; shape (T,6) or (m,T,6)."
    )
    ap.add_argument(
        "--data_pred", type=str, required=True,
        help="Rollout NPZ with sig_pred, epsp_pred, L, H_model, H_test, t0, "
             "and optionally i_spec."
    )
    ap.add_argument(
        "--out_dir", type=str, default="./figs_pq_relax",
        help="Output directory for figure."
    )
    ap.add_argument(
        "--M",  type=float, default=2.0,
        help="MCC slope parameter M."
    )
    ap.add_argument(
        "--pc", type=float, default=-6.0,
        help="MCC preconsolidation pressure pc."
    )
    ap.add_argument(
        "--theta_deg", type=float, default=45.0,
        help="Projection angle θ (deg) for fictitious stress."
    )
    ap.add_argument(
