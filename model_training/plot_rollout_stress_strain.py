...
"""
Usage:
python plot_rollout_stress_strain_multitraj.py \
  --data_truth ../data_generation/.../stress_relaxation_multitraj.npz \
  --data_pred ./rollout_results_L4_H2_Htest20_spec0.npz \
  --out_dir ./figs_stress_strain

This script:
  - loads full ground-truth trajectories from data_truth
      eps, epsp, sig: shape (T,6)           for a single trajectory
                  or: shape (m,T,6)         for multiple trajectories

  - loads rollout predictions from data_pred
      sig_pred, epsp_pred: (H_test, 6)
      L, H_model, H_test, t0
      optionally i_spec: index of specimen used in rollout

  - chooses specimen index:
      if data_pred contains "i_spec", use that;
      otherwise, use CLI arg --i_spec (default 0).
  ...
  Figure 2 (plastic strain–time, 2×3 panels):
    - For I = 0..5 in Voigt order:
        truth:      ε_{p,I}^truth(t), over the entire horizon [0, T)
        prediction: ε_{p,I}^pred(t),  over the rollout horizon [t0, t0+H_test)
"""
...
def main(args):
    ...
    # Determine specimen index
    if "i_spec" in roll.files:
        i_spec_roll = int(roll["i_spec"])
        i_spec = i_spec_roll
        print(f"[INFO] Using i_spec from rollout file: i_spec = {i_spec}")
    else:
        i_spec = args.i_spec
        print(f"[INFO] Rollout file has no 'i_spec'; using CLI i_spec = {i_spec}")
    ...
    # ------------------------------------------------------------
    # Load full ground-truth trajectory (possibly multi-trajectory)
    # ------------------------------------------------------------
    ...
    eps_all  = truth["eps"]    # (T,6) or (m,T,6)
    epsp_all = truth["epsp"]   # (T,6) or (m,T,6)
    sig_all  = truth["sig"]    # (T,6) or (m,T,6)
    ...
    m, T, D = eps_all.shape
    if D != 6:
        raise RuntimeError(f"Expected last dimension = 6 (Voigt-6), got D={D}")

    if not (0 <= i_spec < m):
        raise RuntimeError(f"Requested i_spec={i_spec} out of range for m={m}")

    eps_all_spec  = eps_all[i_spec]   # (T,6)
    epsp_all_spec = epsp_all[i_spec]  # (T,6)
    sig_all_spec  = sig_all[i_spec]   # (T,6)

    print(f"[INFO] Ground truth: m={m}, T={T}, using specimen i_spec={i_spec}")
    ...
    # ------------------------------------------------------------
    eps_roll = eps_all_spec[t0:t0 + H_test]     # (H_test, 6) ε_I^truth on rollout horizon
    # We still use sig_all_spec/epsp_all_spec over [0,T) for "entire horizon" truth curves.
    ...
    fig1.suptitle(
        ...
        f"specimen i_spec={i_spec}, truth: [0, T={T}), "
        f"prediction: [t0={t0}, t0+H_test={t0+H_test}) "
    ...
    for idx, ax in enumerate(axes1.ravel()):
        ...
        x_full = eps_all_spec[:, I]     # ε_I^truth over [0,T)
        y_full = sig_all_spec[:, I]     # σ_I^truth over [0,T)
    ...
    out_path1 = os.path.join(
        args.out_dir,
        f"stress_strain_rollout_voigt_spec{i_spec}_t0_{t0:04d}.png"
    )
    ...
    for idx, ax in enumerate(axes2.ravel()):
        ...
        y_full_epsp = epsp_all_spec[:, I]          # ε_{p,I}^truth over [0,T)

        # Prediction over rollout horizon: epsp vs time step
        y_pred_epsp = epsp_pred_np[:, I]           # ε_{p,I}^pred over [t0,t0+H_test)
    ...
    out_path2 = os.path.join(
        args.out_dir,
        f"epsp_time_rollout_voigt_spec{i_spec}_t0_{t0:04d}.png"
    )
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Plot Voigt-form stress–strain curves comparing rollout prediction "
                    "vs ground truth (multi-trajectory capable)."
    )
    ap.add_argument(
        "--data_truth", type=str, required=True,
        help="Path to ground-truth NPZ with eps, epsp, deps, sig; shape (T,6) or (m,T,6)."
    )
    ap.add_argument(
        "--data_pred", type=str, required=True,
        help="Path to rollout NPZ with sig_pred, epsp_pred, L, H_model, H_test, t0, "
             "and optionally i_spec."
    )
    ap.add_argument(
        "--out_dir", type=str, default="./figs_stress_strain",
        help="Directory for output figure."
    )
    ap.add_argument(
        "--i_spec", type=int, default=0,
        help="Specimen index to use if rollout NPZ does not contain 'i_spec'."
    )
    args = ap.parse_args()
    main(args)
