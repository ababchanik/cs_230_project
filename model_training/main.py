...
# from loss_functions import compute_loss  # Only need compute_loss
...
# def log_epoch_losses(epoch, train_loss, dev_loss, log_path="train_log.txt"):
#     now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#     msg = f"[{now}] Epoch {epoch:03d} | train {train_loss:.6f} | dev {dev_loss:.6f}"
#     with open(log_path, "a") as f:
#         f.write(msg + "\n")
#     print(msg)
#     return msg
...
#     # Load datasets --------------------------------
#     ds_tr = TrajWindowDataset(args.data, L=args.L, H=args.H, split="train")
#     ds_va = TrajWindowDataset(args.data, L=args.L, H=args.H, split="dev")

#     tr = DataLoader(ds_tr, batch_size=args.bs, shuffle=True, num_workers=1)
#     va = DataLoader(ds_va, batch_size=args.bs, shuffle=False, num_workers=1)
...
#         for i, (eps, epse, deps, sig_y, epse_y) in enumerate(tr):
...
#             # Debug check (first batch, first epoch)
#             if epoch == 1 and i == 0:
#                 print("=== STRESS SCALING CHECK ===")
#                 print(f"sig_true stats:")
#                 print(f"  Shape: {sig_y.shape}")
#                 print(f"  Min: {sig_y.min().item():.6f}")
#                 print(f"  Max: {sig_y.max().item():.6f}")
#                 print(f"  Range check: [{sig_y.min().item():.6f}, {sig_y.max().item():.6f}]")
                
#                 print(f"\nsig_pred stats:")
#                 print(f"  Min: {sig_pred.min().item():.6f}")
#                 print(f"  Max: {sig_pred.max().item():.6f}")
#                 print(f"  Range check: [{sig_pred.min().item():.6f}, {sig_pred.max().item():.6f}]")
                
#                 # Check stress values are reasonable
#                 print(f"\nStress value check:")
#                 print(f"  sig_true mean: {sig_y.mean().item():.6f}")
#                 print(f"  sig_true std: {sig_y.std().item():.6f}")
#                 print(f"  sig_pred mean: {sig_pred.mean().item():.6f}")
#                 print(f"  sig_pred std: {sig_pred.std().item():.6f}")
#                 print("============================\n")

#             # Compute loss - NO scaling_factors needed
#             loss = compute_loss(
#                 sig_pred=sig_pred,
#                 sig_true=sig_y,
#                 loss_type=args.loss,
#                 epse_pred=epse_pred,
#                 epse_true=epse_y,
#                 # No scaling_factors parameter for Lie loss
#             )
            
#             # Check if loss is valid
#             if torch.isnan(loss) or torch.isinf(loss):
#                 print(f"Warning: Invalid loss value {loss.item()} at epoch {epoch}, batch {i}")
#                 continue
...
#         with torch.no_grad():
#             for eps, epse, deps, sig_y, epse_y in va:
#                 eps, epse, deps = [x.to(device) for x in (eps, epse, deps)]
#                 sig_y, epse_y = sig_y.to(device), epse_y.to(device)
...
#         dev_avg   = vloss / len(va)
#         log_epoch_losses(epoch, train_avg, dev_avg)
...
#         default="lie",
#         choices=["voigt", "lie"],  
#         help="Loss function type: 'lie' (geodesic) or 'voigt' (component-based)"
#     )
