...
LAMBDAS=("1e3" "1e4" "1e5" "1e6" "1e7" "1e8" "1e9" "1e10")
...
for LAMBDA_P in "${LAMBDAS[@]}"; do
  ...
  TAG="lamP_${LAMBDA_P}_L${L_STR}_H${H_STR}_S${S_STR}_Htest${H_TEST_STR}"
  ...
  echo "[OK] Training completed for lambda_pstrain=${LAMBDA_P}."
