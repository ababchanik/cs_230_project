...
def eval_single_batch(model, dataloader, args, device, CCe_voigt):
    ...
    with torch.no_grad():
        ...
        eps, epsp, deps, sig_y, epsp_y, t0 = [x.unsqueeze(0) if torch.is_tensor(x) else x for x in dataloader.dataset[0]] # Fixed
        eps, epsp, deps = [x.to(device) for x in (eps, epsp, deps)]    # (B,L+S,6)
        sig_y, epsp_y   = [x.to(device) for x in (sig_y, epsp_y)]      # (B,H+S,6)

        L, H, S = args.L, args.H, args.S

        # Same slices as during training
        ...
        # Rebuild σ_hist using history only
        sig_hist = elastic_update(eps_hist - epsp_hist, CCe_voigt)   # (B,L,6)

        # Forward
        sig_pred, epsp_pred = model(eps_hist, epsp_hist, deps_hist, sig_hist)
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for eps, epsp, deps, sig_y, epsp_y, t0 in tr:
            ...
            eps, epsp, deps = [x.to(device) for x in (eps, epsp, deps)]      # (B,L+S,6), L_total = L + S
            sig_y, epsp_y   = [x.to(device) for x in (sig_y, epsp_y)]        # (B,H+S,6), H_total = H + S
            ...
            sig_pred, epsp_pred = model(eps_hist, epsp_hist, deps_hist, sig_hist)  # (B,H,6)
            ...
            if (args.S > 0) and (args.lambda_self > 0.0):
                # history clone
                ...
                sig_self_hist = elastic_update(eps_self - epsp_self, CCe_voigt)    # (B,L,6)
                sig_self, epsp_self_pred = model(eps_self, epsp_self, deps_self, sig_self_hist)  # (B,H,6)

                # shift ground truth for comparison
                sig_y_S  = sig_y_H.clone()
                epsp_y_S = epsp_y_H.clone()
                sig_y_S[:, :-S, :]  = sig_y_S[:, S:, :]          # shift left by S
                sig_y_S[:, -S:, :]  = sig_y[:, H:, :]            # append ground truth sig_y(H+1:H+S)
                epsp_y_S[:, :-S, :] = epsp_y_S[:, S:, :]         # shift left by S
                epsp_y_S[:, -S:, :] = epsp_y[:, H:, :]           # append ground truth epsp_y(H+1:H+S)
                
                # compare from input  (t - L + 1 + S) to (t + S)
                #              output (t + 1 + S) to (t + H + S)
                loss_self = mse_loss(sig_self, sig_y_S) \
                            + args.lambda_pstrain * mse_loss(epsp_self_pred, epsp_y_S)
