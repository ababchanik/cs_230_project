...
class SeqModel(nn.Module):
    """
    Per-step inputs (features): total strain eps(6) + plastic strain epsp(6) + increment deps(6) + trial stress(6) = 24
    Per-step outputs (forecast): stress sig(6) + plastic strain epsp(6) = 12
    """
    ...
    def _step_features(self, eps, epsp, deps, sig_trial):
        # eps, epsp, deps: (B, L, 6)
        return torch.cat([eps, epsp, deps, sig_trial], dim=-1)  # (B, L, 24)

    def forward(self, eps_seq, epsp_seq, deps_seq, sig_trial_seq):
        # eps_seq, epsp_seq, deps_seq: (B, L, 6)
        # X from (t - L + 1) to (t)
        X = self._step_features(eps_seq, epsp_seq, deps_seq, sig_trial_seq)  # (B, L, 24)
        # h at (t)
        h = self.enc(X)                                                      # (B, hid)
        # Y from (t + 1) to (t + H)
        Y = self.dec(h)                                                      # (B, H, 12)
