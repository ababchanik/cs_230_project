...
- Uses model with inputs (eps_in, epsp_in, deps_in, sig_trial_in)
...
- Saves ONLY predicted sig_pred and epsp_pred + metadata (L, H_model, H_test, t0)
...
def main(args):
    ...
    # Rolling buffer for epsp
    epsp_roll = epsp_all.copy()
    ...
    for k in range(n_blocks):
        ...
        epsp_in = torch.from_numpy(epsp_roll[in_start:in_end]).float().unsqueeze(0).to(device)

        # Trial
        sig_hist_in = elastic_update(eps_in - epsp_in, CCe_voigt)

        # Model rollout
        with torch.no_grad():
            sig_pred_block, epsp_pred_block = model(eps_in, epsp_in, deps_in, sig_hist_in)

        sig_pred_block_np  = sig_pred_block.squeeze(0).cpu().numpy()
        epsp_pred_block_np = epsp_pred_block.squeeze(0).cpu().numpy()
    ...
    np.savez(
        ...
        epsp_pred=epsp_pred_roll,      # (H_test, 6)
        L=args.L,
        H_model=H_model,
        H_test=H_test,
        t0=t0,
