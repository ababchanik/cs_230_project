...
This version is PLOT-ONLY:
  ...
  • loads full eps, epse, deps, sig from a .npz WITHOUT 'split' (data_truth)
  • loads rollout predictions from data_pred (sig_pred, epse_pred, L, H_model, H_test, t0)
...
def main(args):
    ...
    # epse_pred_np = roll["epse_pred"]  # unused here
    L       = int(roll["L"])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--data_truth", type=str, required=True, help="Ground-truth NPZ with eps, epse, deps, sig (single trajectory).")
    ap.add_argument("--data_pred", type=str, required=True, help="Rollout NPZ with sig_pred, epse_pred, L, H_model, H_test, t0.")
